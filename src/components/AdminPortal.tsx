      ...
      <header className="bg-white text-slate-900 px-4 py-3 flex items-center justify-between border-b border-slate-200">
        ...
        <div className="flex items-center space-x-4">
          <div className="text-right hidden sm:block leading-none">
            <p className="text-xs font-bold text-slate-900 font-mono uppercase">{adminConfig.adminUsername}</p>
            <span className="text-[8px] text-indigo-600 font-mono font-bold uppercase tracking-wider">Root Active</span>
          </div>

          <button 
            onClick={onLogOut}
            id="btn-admin-logout"
            className="flex items-center space-x-1 px-3 py-1.5 bg-slate-50 hover:bg-rose-500/25 hover:text-rose-400 border border-slate-200 hover:border-rose-500/50 text-[10px] font-mono uppercase tracking-wider transition-all cursor-pointer"
          >
      ...
      <nav className="bg-white border-b border-slate-200 px-4 py-0 flex items-center justify-between">
        <div className="flex space-x-2">
          <button
            ...
            id="tab-requests"
            className={`py-3 px-4 font-mono font-bold text-xs border-b-2 transition-all uppercase tracking-wider cursor-pointer ${
              activeTab === 'requests' 
        ...
        <div className="text-slate-900/35 font-mono text-[9px] font-bold hidden md:block tracking-widest uppercase">
            ...
            <div className="lg:col-span-5 bg-white border border-slate-200 p-4 flex flex-col max-h-[700px] text-slate-900">
              ...
              <div className="space-y-3 mb-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-xs font-bold uppercase text-slate-900 tracking-widest font-mono">Proposals</h2>
                  <span className="px-2 py-0.5 bg-indigo-50 border border-indigo-200 text-indigo-600 font-bold font-mono text-[9px] uppercase tracking-widest">
                ...
                <div className="flex flex-wrap gap-1 p-1 bg-slate-50 border border-slate-200">
                  {['all', 'pending', 'approved', 'rejected', 'payment_submitted', 'completed'].map((status) => (
                    <button
                      ...
                      className={`px-2 py-1 text-[9px] font-bold font-mono uppercase tracking-wider transition-all cursor-pointer ${
                        statusFilter === status 
                  ...
                  filteredRequests.map((req) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          <p className="font-bold text-slate-900 text-xs uppercase truncate">{req.name}</p>
                ...
                <div className="flex-grow flex flex-col h-full overflow-hidden">
                  ...
                  <div className="p-3.5 border-b border-slate-200 flex items-center justify-between bg-slate-50">
                    <div>
                      <div className="flex items-center space-x-1.5">
                        <h3 className="font-extrabold text-slate-900 text-xs uppercase">{selectedRequest.name}</h3>
                    ...
                    <div className="flex items-center space-x-1.5">
                      <button 
                        ...
                        className="p-1.5 px-3 bg-indigo-600 hover:bg-indigo-700 text-white hover:text-indigo-600 transition-colors font-mono text-[9px] font-bold uppercase cursor-pointer flex items-center space-x-1"
                  ...
                  <div className="flex-grow overflow-y-auto p-4 space-y-4">
                    ...
                    <div className="bg-slate-50 p-4 border border-slate-200">
                      <h4 className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-2 font-mono">Proposal requirements</h4>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 pt-4 border-t border-slate-200 text-[10px] font-mono">
                        <div>
                          <p className="text-slate-400 font-bold uppercase tracking-wider text-[8px] font-mono">WHATSAPP</p>
                          <p className="text-slate-900 font-bold mt-1 text-xs">{selectedRequest.whatsapp}</p>
                        </div>
                        {selectedRequest.companyName && (
                          <div>
                            <p className="text-slate-400 font-bold uppercase tracking-wider text-[8px] font-mono">COMPANY</p>
                            <p className="text-slate-900 font-bold mt-1 text-xs truncate uppercase">{selectedRequest.companyName}</p>
                          </div>
                        )}
                        <div>
                          <p className="text-slate-400 font-bold uppercase tracking-wider text-[8px] font-mono">CLIENT BUDGET</p>
                          <p className="text-indigo-600 font-black mt-1 text-xs">
                            {selectedRequest.budgetCurrency === 'USD' ? '$' : '₹'}{selectedRequest.budgetAmount.toLocaleString()}
                          </p>
                        </div>
                        <div>
                          <p className="text-slate-400 font-bold uppercase tracking-wider text-[8px] font-mono">SUBMITTED</p>
                    ...
                    {(selectedRequest.status === 'approved' || selectedRequest.status === 'payment_submitted' || selectedRequest.status === 'completed') && (
                      <div className="bg-indigo-50 border border-indigo-200 p-3">
                        <div className="flex justify-between items-center">
                          <div>
                            <p className="text-indigo-600 text-[9px] font-bold uppercase tracking-wider font-mono">Approved final pricing</p>
                            <p className="text-sm font-display font-black text-slate-900 mt-0.5">
                              {selectedRequest.approvedCurrency === 'USD' ? '$' : '₹'}{selectedRequest.approvedAmount?.toLocaleString()}
                            </p>
                          </div>
                          <span className="px-2 py-0.5 bg-indigo-600 text-white text-[9px] font-bold uppercase tracking-wider font-mono">
                            APPROVED
                          </span>
                        </div>
                      </div>
                    ...
                    {(selectedRequest.status === 'payment_submitted' || selectedRequest.status === 'completed') && (
                      <div className="bg-blue-500/10 border border-blue-500/20 p-3.5 space-y-2.5">
                        <h4 className="text-[9px] font-extrabold text-blue-400 uppercase tracking-widest font-mono">Payment transaction proof</h4>
                    ...
                    <div className="border-t border-slate-200 pt-4 space-y-3">
                        ...
                        <div className="space-y-3.5 bg-slate-50 p-4 border border-slate-200">
                          ...
                          <div className="flex gap-3 items-end">
                            <div className="flex-1">
                              <label className="block text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 font-mono">FINAL APPROVED PRICE</label>
                              <div className="flex border border-slate-300 bg-white">
                                <select 
                                  ...
                                  className="bg-slate-50 text-slate-900 font-bold px-2 py-1.5 text-[10px] focus:outline-none border-r border-slate-300 font-mono uppercase"
                                >
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white hover:text-indigo-600 text-[10px] font-mono font-bold uppercase tracking-widest border border-indigo-600 transition-colors flex items-center space-x-1 cursor-pointer"
                            >
                            ...
                            <button
                              ...
                              id="btn-admin-reject"
                              className="px-3 py-2 bg-slate-50 hover:bg-rose-500/20 border border-slate-200 hover:border-rose-500/45 text-rose-400 text-[10px] font-mono font-bold uppercase tracking-widest transition-colors flex items-center space-x-1 cursor-pointer"
                            >
                      ...
                      {selectedRequest.status === 'approved' && (
                        <div className="bg-slate-50 p-4 border border-slate-200 text-center text-slate-500 text-[10px] py-6 space-y-2">
                          ...
                          <p className="font-bold text-slate-900 uppercase font-mono tracking-wider">Awaiting Client Payment Execution</p>
                      ...
                      {selectedRequest.status === 'payment_submitted' && (
                        <div className="bg-indigo-50 border border-indigo-200 p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                          <div className="text-left space-y-1">
                            <h5 className="font-bold text-indigo-600 text-xs flex items-center space-x-1.5 uppercase font-mono">
                          ...
                          <div className="flex gap-2 w-full sm:w-auto">
                            <button
                              ...
                              className="w-full sm:w-auto px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white hover:text-indigo-600 text-[10px] font-mono font-bold uppercase tracking-widest border border-indigo-600 transition-colors flex items-center justify-center space-x-1 cursor-pointer"
                            >
                              <CheckCircle className="w-3.5 h-3.5" />
                      ...
                      {selectedRequest.status === 'completed' && (
                        <div className="bg-indigo-50 text-indigo-600 border border-indigo-200 p-3 text-[10px] font-bold font-mono flex items-center space-x-3">
                          ...
                          <div>
                            <p className="uppercase tracking-wider">[ TRANSACTION FULLY VERIFIED & COMPLETE ]</p>
                            <p className="text-slate-500 mt-1 uppercase">Kicked off: {selectedRequest.paymentVerifiedAt ? new Date(selectedRequest.paymentVerifiedAt).toLocaleString() : 'N/A'}</p>
                          </div>
                        </div>
                      )}
                      ...
                      {selectedRequest.status === 'rejected' && (
                        <div className="bg-rose-500/10 text-rose-400 border border-rose-500/30 p-3 text-[10px] font-bold font-mono flex items-center space-x-3">
                          ...
                          <div>
                            <p className="uppercase tracking-wider">Request declined</p>
        ...
        {activeTab === 'chat' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-start h-full max-h-[700px]">
            ...
            <div className="lg:col-span-4 bg-white border border-slate-200 p-3 flex flex-col h-[600px] overflow-hidden">
              <h3 className="text-xs font-extrabold uppercase text-slate-900 mb-3 flex items-center space-x-1.5 tracking-widest font-mono">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-grow min-w-0 font-mono">
                          <div className="flex justify-between items-center">
                            <span className="font-bold text-slate-900 text-xs truncate uppercase">{req.name}</span>
                            <span className="text-[8px] font-mono font-bold text-indigo-600 uppercase bg-slate-50 px-1.5 py-0.5 border border-indigo-200">
                              {req.id}
                            </span>
                          </div>
                ...
                <div className="flex-grow flex flex-col h-full overflow-hidden">
                  ...
                  <div className="p-3 border-b border-slate-200 flex items-center justify-between bg-slate-50 font-mono">
                    <div className="flex items-center space-x-2.5">
                      <div className="w-8 h-8 bg-indigo-600 text-white flex items-center justify-center font-black text-xs">
                        {selectedRequest.name.substring(0,2).toUpperCase()}
                      </div>
                      <div>
                        <h4 className="font-bold text-slate-900 text-xs uppercase">SECURE SESSION WITH {selectedRequest.name}</h4>
                        <p className="text-slate-400 text-[9px] font-mono mt-0.5">REFERENCE: {selectedRequest.id} | WHATSAPP: {selectedRequest.whatsapp}</p>
                      </div>
                    </div>

                    <span className={`px-2 py-0.5 text-[8px] font-extrabold uppercase border ${
                  ...
                  <div className="flex-grow overflow-y-auto p-4 bg-slate-50 space-y-3">
                    {chatMessages.length === 0 ? (
                      <div className="text-center py-24 text-slate-300 text-xs font-mono uppercase">[ NO LOGS EXIST. SAY HELLO FIRST ]</div>
                    ) : (
          ...
          <div className="max-w-2xl mx-auto bg-white border border-slate-200 p-6 relative overflow-hidden">
            ...
            <div className="space-y-6">
              <div className="border-b border-slate-200 pb-4">
                <h2 className="text-xs font-extrabold uppercase text-slate-900 flex items-center space-x-1.5 tracking-widest font-mono">
                  <Settings className="w-4 h-4 text-indigo-600" />
                  <span>Platform system settings</span>
                </h2>
                <p className="text-slate-500 text-[10px] mt-1 font-mono uppercase">Manage admin credentials, update payment gateway settings, adjust standard plans, and upload UPI QR codes.</p>
              </div>

              {settingsSuccess && (
                <div className="bg-indigo-50 text-indigo-600 border border-indigo-200 p-3 flex items-center space-x-2 text-[10px] font-mono uppercase font-bold">
              ...
              <form onSubmit={handleSaveSettings} className="space-y-6">
                ...
                <div className="space-y-4">
                  <h3 className="text-[9px] font-extrabold text-indigo-600 uppercase tracking-widest font-mono">1. ADMIN CREDENTIALS & SECURE ACCESS PATH</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-slate-500 text-[9px] font-bold uppercase tracking-widest mb-1.5 font-mono">
                        System Username
                      </label>
                      <input 
                        type="text"
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-slate-400 mt-1.5 leading-relaxed font-mono uppercase">
                ...
                <div className="space-y-4 pt-4 border-t border-slate-200">
                  <h3 className="text-[9px] font-extrabold text-indigo-600 uppercase tracking-widest font-mono">2. UPI PAYMENT GATEWAY CONFIG</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                    ...
                    <div className="md:col-span-7 space-y-4">
                      <div>
                        ...
                        />
                        <p className="text-[9px] text-slate-400 mt-1.5 leading-relaxed font-mono uppercase">When empty or default, we render a dynamic UPI Pay URL scannable by BHIM, GooglePay, and PhonePe.</p>
                      </div>

                      <div className="p-3 bg-slate-50 border border-slate-200 flex items-start space-x-2 text-[9px] text-slate-500 font-mono uppercase">
                    ...
                    <div className="md:col-span-5 bg-slate-50 p-4 border border-slate-200 text-center space-y-3 flex flex-col items-center">
                      <p className="text-[9px] font-bold text-slate-500 uppercase font-mono tracking-wider">UPI QR Code</p>
                      ...
                      {qrFileBase64 ? (
                        ...
                        <div className="w-24 h-24 border border-dashed border-slate-300 flex flex-col items-center justify-center bg-slate-50 text-slate-300 text-xs p-2">
                          ...
                          <span className="text-[8px] font-mono uppercase">NO IMAGE</span>
                        </div>
                      )}

                      <div className="w-full">
                        <label className="block w-full py-1.5 bg-slate-50 hover:bg-white border border-slate-200 text-[9px] font-bold uppercase cursor-pointer text-slate-900 hover:text-indigo-600 transition-colors font-mono">
                          <input 
                            type="file"
                            accept="image/*"
                        ...
                        <p className="text-[8px] text-slate-400 mt-1 font-mono uppercase">Stored as secure inline data-URL.</p>
                      </div>
                    </div>
                ...
                <div className="space-y-4 pt-4 border-t border-slate-200 font-mono">
                  <h3 className="text-[9px] font-extrabold text-indigo-600 uppercase tracking-widest font-mono">3. PRICE TABLE UPDATES</h3>
                  <p className="text-[10px] text-slate-500 leading-relaxed uppercase">Adjust standard starter template pricing. Changes are applied immediately to standard plan cards on the client landing page.</p>

                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      <label className="block text-slate-500 text-[9px] font-bold uppercase tracking-widest mb-1.5">
                        Starter (INR)
                      </label>
                      <input 
                        type="number"
                  ...
                  <div className="pt-4 border-t border-slate-200 flex justify-end">
                    <button
                      ...
                      className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white hover:text-indigo-600 disabled:bg-slate-50 disabled:text-slate-200 disabled:border-slate-200 text-[10px] font-bold uppercase tracking-widest border border-indigo-600 transition-colors flex items-center space-x-1.5 cursor-pointer"
                    >
                      {updatingSettings ? (
                        <>
